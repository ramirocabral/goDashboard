  ...
  const formatSataData = (data) => {
    ...
    const rows = Object.entries(data)
      ...
      .map(([_, value]) => {
        ...
        return values
          .map((val, i) =>
            val.padEnd(i < columnWidths.length ? columnWidths[i] : 0),
          )
          .join(" ");
      });
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              {smartData.devices.length} disks monitored
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {smartData.devices.map((device, index) => {
          return (
            <div
              key={index}
              className="border border-gray-700 rounded-md overflow-hidden bg-gray-800/50"
            >
              <div
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-2">
                    ...
                    <div className="text-xs text-gray-400">
                      {device.type === "sata" ? "SATA Drive" : "NVMe Drive"}
                    </div>
                  </div>
                </div>
                <div className="flex items-center">
              ...
              {expandedDisk === device.device && (
                <div className="p-3 border-t border-gray-700 bg-gray-800/30">
                  <pre className="font-mono text-xs text-gray-200 whitespace-pre overflow-x-auto bg-[#1a1b26] p-4 rounded">
                    {device.type === "sata"
                      ? formatSataData(device.data)
                      : formatNvmeData(device.data)}
                  </pre>
                </div>
              )}
            </div>
